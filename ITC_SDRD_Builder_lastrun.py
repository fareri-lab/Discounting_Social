...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/MichaelPoon/Desktop/Psychology/SDRD Project/Intertemporal Choice Task/GitHub Discounting Social/Discounting_Social/ITC_SDRD_Builder_lastrun.py',
...
for thisLoop in Loop:
    ...
    while continueRoutine:
        ...
        if t >= 0.0 and leftbox.status == NOT_STARTED:
            # keep track of start time/frame for later
            leftbox.tStart = t  # not accounting for scr refresh
        ...
        if t >= 0.0 and rightbox.status == NOT_STARTED:
            # keep track of start time/frame for later
            rightbox.tStart = t  # not accounting for scr refresh
        ...
        if t >= 0.0 and responses.status == NOT_STARTED:
            # keep track of start time/frame for later
            responses.tStart = t  # not accounting for scr refresh
            ...
            responses.clearEvents(eventType='keyboard')
        ...
        if responses.status == STARTED:
            ...
            if len(theseKeys):
                ...
                if "escape" == theseKeys:
                    endExpNow = True
                responses.keys = theseKeys.name  # just the last key pressed
                responses.rt = theseKeys.rt
        
        # *money1* updates
        if t >= 0.0 and money1.status == NOT_STARTED:
            # keep track of start time/frame for later
            money1.tStart = t  # not accounting for scr refresh
            money1.frameNStart = frameN  # exact frame index
            win.timeOnFlip(money1, 'tStartRefresh')  # time at next scr refresh
            money1.setAutoDraw(True)
        
        # *money2* updates
        if t >= 0.0 and money2.status == NOT_STARTED:
            # keep track of start time/frame for later
            money2.tStart = t  # not accounting for scr refresh
            money2.frameNStart = frameN  # exact frame index
            win.timeOnFlip(money2, 'tStartRefresh')  # time at next scr refresh
        ...
        if Immed_Resp.status == STARTED and t >= (Immed_Resp.tStart + 2):
            # keep track of stop time/frame for later
            Immed_Resp.tStop = t  # not accounting for scr refresh
            Immed_Resp.frameNStop = frameN  # exact frame index
            win.timeOnFlip(Immed_Resp, 'tStopRefresh')  # time at next scr refresh
        ...
        if Delay_Resp.status == STARTED and t >= (Delay_Resp.tStart + 2):
            # keep track of stop time/frame for later
            Delay_Resp.tStop = t  # not accounting for scr refresh
            Delay_Resp.frameNStop = frameN  # exact frame index
            win.timeOnFlip(Delay_Resp, 'tStopRefresh')  # time at next scr refresh
    ...
    for thisComponent in ChoiceComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    Loop.addData('leftbox.started', leftbox.tStartRefresh)
    Loop.addData('leftbox.stopped', leftbox.tStopRefresh)
    Loop.addData('rightbox.started', rightbox.tStartRefresh)
    Loop.addData('rightbox.stopped', rightbox.tStopRefresh)
    # check responses
    ...
    Loop.addData('responses.started', responses.tStartRefresh)
    Loop.addData('responses.stopped', responses.tStopRefresh)
    Loop.addData('money1.started', money1.tStartRefresh)
    Loop.addData('money1.stopped', money1.tStopRefresh)
    Loop.addData('money2.started', money2.tStartRefresh)
    Loop.addData('money2.stopped', money2.tStopRefresh)
    Loop.addData('blank.started', blank.tStartRefresh)
    Loop.addData('blank.stopped', blank.tStopRefresh)
    ...
    for thisComponent in ISIComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    Loop.addData('text_3.started', text_3.tStartRefresh)
    Loop.addData('text_3.stopped', text_3.tStopRefresh)
    thisExp.nextEntry()
