...
function experimentInit() {
  ...
  Instructions2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'Instructions',
    text: 'The choices you will be making are hypothetical, but please treat them as if they are real. \n\nWhen you are ready, press the SPACEBAR to start!',
    font: 'Arial',
  ...
  ImmedMoney = new visual.TextStim({
    win: psychoJS.window,
    name: 'ImmedMoney',
    text: 'default text',
  ...
  DelayMoney = new visual.TextStim({
    win: psychoJS.window,
    name: 'DelayMoney',
    text: 'default text',
    font: 'Times New Roman',
...
//   });
 ...
 /* 
  ...
  Question2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'Question2',
    text: 'How likely were you to choose more immediate rewards during this task compared to the first time you completed the task?',
    font: 'Arial',
    pos: [0, 0.2], height: 0.08,  wrapWidth: 1.2, ori: 0,
    color: new util.Color('white'),  opacity: 1,
    depth: 0.0 
...
/*  
  ...
  Question3 = new visual.TextStim({
    win: psychoJS.window,
    name: 'Question3',
    text: 'How likely were you to choose more delayed rewards during this task compared to the first time you completed the task??',
    font: 'Arial',
    pos: [0, 0.2], height: 0.08,  wrapWidth: 1.2, ori: 0,
    color: new util.Color('white'),  opacity: 1,
    depth: 0.0 
...
function LoopLoopBegin(thisScheduler) {
  ...
  Loop = new TrialHandler({
    ...
    trialList: 'IntertemporalChoice_17item_test_v2.csv',
    //trialList: 'ITC2.csv',
...
function ChoiceRoutineBegin() {
  ...
  frameN = -1;
  // update component parameters for each repeat
  ImmedMoney.setText(Immed_Choice_Text);
  DelayMoney.setText(Delay_Choice_Text);
...
//  TimeText.setText(Time);
  ...
  // keep track of which components have finished
  ChoiceComponents = [];
  ChoiceComponents.push(ImmedMoney);
  ChoiceComponents.push(DelayMoney);
...
function ChoiceRoutineEachFrame() {
 ...
 // responseTimer.reset();
  ...
  // *ImmedMoney* updates
  if (t >= 0.0 && ImmedMoney.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
    ImmedMoney.tStart = t;  // (not accounting for frame time here)
    ImmedMoney.frameNStart = frameN;  // exact frame index
    ImmedMoney.setAutoDraw(true);
  }

  
  // *DelayMoney* updates
  if (t >= 0.0 && DelayMoney.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
    DelayMoney.tStart = t;  // (not accounting for frame time here)
    DelayMoney.frameNStart = frameN;  // exact frame index
    DelayMoney.setAutoDraw(true);
  }
...
//   }
  ...
  if (responses.keys == '1') {
      ImmedMoney.setColor(new util.Color(DecisionColor));
//      NowText.setColor(new util.Color(DecisionColor));
  }
  
  if (responses.keys == '2') {
      DelayMoney.setColor(new util.Color(DecisionColor));
...
//   show the response for 2 seconds, then move on to next trial
 if (conditionalBlank.status == PsychoJS.Status.FINISHED){
    ImmedMoney.setAutoDraw(false);
    DelayMoney.setAutoDraw(false);
...
function ChoiceRoutineEnd() {
  ...
  ImmedMoney.setColor(new util.Color("white"));
  DelayMoney.setColor(new util.Color("white"));
